...
All assertions are made through HTTP endpoints only — no direct inspection
of internal state (e.g. _node_state). Uses TestClient against the full app
from create_guest_app.

Tests cover:
- Gradient share shape and dtype
- Order enforcement (find_split before gradient_shares → 404)
- State cleared after apply_split
- Node isolation (apply_split for one node does not clear another)
- Happy-path end-to-end protocol round (gradient_shares → find_split)
"""
...
labels: npt.NDArray[np.float64] = _rng.integers(0, 2, size=N_SAMPLES).astype(np.float64)
...
class TestGradientShares:
    ...
    def test_shares_are_int64(self, sample_indices: list[int]) -> None:
        with TestClient(app) as client:
            req = GradientShareRequest(node_id="node_0", sample_indices=sample_indices)
            resp = _post(client, "/gradient_shares", req)
    ...
    def test_different_nodes_store_separate_state(self, sample_indices: list[int]) -> None:
        with TestClient(app) as client:
...
# ---------------------------------------------------------------------------
# Tests: order enforcement
# ---------------------------------------------------------------------------


def test_find_split_without_gradient_shares_returns_404() -> None:
    """find_split before gradient_shares must return 404."""
    with TestClient(app) as client:
        resp = client.post(
            "/find_split",
            json={
...
def test_apply_split_clears_state_for_node(
    feature_data: dict[str, npt.NDArray[np.float64]],
    sample_indices: list[int],
) -> None:
    """After apply_split, find_split for the same node_id must return 404."""
    node_id = "node_clear"
    ...
    with TestClient(app) as client:
        ...
        # Now find_split should 404 — state was cleared
...
def test_apply_split_does_not_clear_other_nodes(
    feature_data: dict[str, npt.NDArray[np.float64]],
    sample_indices: list[int],
) -> None:
    """apply_split for node-0 must not clear node-1's state."""
    with TestClient(app) as client:
        # Register gradient shares for both nodes
        for nid in ("node-0", "node-1"):
            grad_req = GradientShareRequest(node_id=nid, sample_indices=sample_indices)
            resp = _post(client, "/gradient_shares", grad_req)
            assert resp.status_code == 200, resp.text

        # Clear only node-0
        apply_req = ApplySplitRequest(node_id="node-0", feature_id="f0", threshold=0.5)
        apply_resp = _post(client, "/apply_split", apply_req)
        assert apply_resp.status_code == 200

        # find_split for node-1 should still work (state intact)
        grad_resp_raw = _post(
            client,
            "/gradient_shares",
            GradientShareRequest(node_id="node-1-recheck", sample_indices=sample_indices),
        )
    ...
    assert find_resp.status_code != 404, (
        "node-1 state should still exist after apply_split cleared node-0"
    )
...
class TestFindSplit:
    def test_returns_valid_split_decision(
        ...
        with TestClient(app) as client:
            ...
            # Step 2: simulate host computing histogram shares from the JSON response
            g_share_a = grad_resp.g_share_a.to_array().astype(np.int64)
            h_share_a = grad_resp.h_share_a.to_array().astype(np.int64)
            host_feature_shares, bucket_indices_per_feature = _simulate_host_histogram_shares(
                g_share_a=g_share_a,
...
class TestApplySplit:
    def test_apply_split_returns_empty_dict(self, sample_indices: list[int]) -> None:
        with TestClient(app) as client:
    ...
    def test_apply_split_on_unknown_node_is_idempotent(self) -> None:
        with TestClient(app) as client:
